...
> **Dormant (ADR-026, Ranti rent-only).** Les charges variables sont retirées :
> les RPC charge ci-dessous (`add_lease_charge`, `withdraw_ledger_line`,
> `replace_ledger_charge`, `*_ledger_line_by_token`), la colonne `tenant_token`,
> les types `reparation`/`frais`, la vue `ops_ledger_notifications` et les
> colonnes `pending_debits` / `disputed_debits` de `lease_balances` ne sont plus
> écrites ni lues par l'app (valeur 0). Objets conservés en base, drop planifié.
