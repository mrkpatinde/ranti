...
- **trancher l'ambiguïté des références RNT — le problème est LIVE.** La prod porte déjà deux `RNT-2026-0001` (un par bailleur) et deux `R-000001` : la recherche par référence sur `/verifier` renvoie donc **déjà** « plusieurs documents portent cette référence » pour le numéro le plus courant. Ce n'est plus un risque théorique ;
...
- hydrater la progression FirstRun depuis les données (un rechargement de `/first-run` repart d'un état vide et peut faire recréer un bail existant) ;
