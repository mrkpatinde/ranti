  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col px-6 py-8">
      <header className="flex items-center justify-between gap-4 border-b border-neutral-200 pb-5 dark:border-neutral-800">
        ...
        <Link href="/leases" className="text-sm font-medium text-neutral-600 underline-offset-4 hover:underline dark:text-neutral-300">Tous les baux</Link>
      </header>
      ...
      <section className="flex flex-1 flex-col gap-8 py-10">
        ...
        <div className="rounded-3xl border border-neutral-200 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-950">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">{formatAmount(lease.monthly_rent_amount)} / mois</h1>
              <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">{tenant ? `${tenant.first_name} ${tenant.last_name}` : "Locataire"} — {unit?.name ?? "Logement"}</p>
              <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Échéance le {lease.due_day} · début {formatDate(lease.start_date)}{lease.end_date ? ` · fin ${formatDate(lease.end_date)}` : ""}</p>
            </div>
            <span className="shrink-0 rounded-lg border border-neutral-300 px-3 py-1.5 text-xs font-medium text-neutral-700 dark:border-neutral-700 dark:text-neutral-200">{leaseStatusLabels[lease.status]}</span>
          </div>

          {lease.notes ? <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-400">{lease.notes}</p> : null}

          <div className="mt-5 flex flex-wrap gap-3">
            {lease.status === "draft" ? (
              <>
                <Link href={`/leases/${lease.id}/edit`} className="rounded-xl border border-neutral-300 px-5 py-2.5 text-sm font-medium text-neutral-800 transition hover:border-neutral-950 dark:border-neutral-700 dark:text-neutral-100 dark:hover:border-neutral-50">Modifier le bail</Link>
                <form action={activateLease}>
                  <input type="hidden" name="id" value={lease.id} />
                  <SubmitButton className="rounded-xl bg-neutral-950 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-neutral-800 disabled:opacity-60 dark:bg-neutral-50 dark:text-neutral-950 dark:hover:bg-neutral-200">Activer et générer les loyers</SubmitButton>
                </form>
              </>
            ) : null}
            {lease.status === "active" ? (
              <form action={endLease}>
                <input type="hidden" name="id" value={lease.id} />
                <SubmitButton className="rounded-xl border border-neutral-300 px-5 py-2.5 text-sm font-medium text-neutral-800 transition hover:border-neutral-950 disabled:opacity-60 dark:border-neutral-700 dark:text-neutral-100 dark:hover:border-neutral-50">Terminer le bail</SubmitButton>
              </form>
            ) : null}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">Échéances de loyer</h2>
          {dues.length === 0 ? (
            <p className="text-sm text-neutral-500 dark:text-neutral-400">{lease.status === "draft" ? "Aucune échéance. Activez le bail pour les générer." : "Aucune échéance pour ce bail."}</p>
          ) : (
            <div className="space-y-3">
              {dues.map((due) => (
                <article key={due.id} className="flex items-center justify-between gap-4 rounded-2xl border border-neutral-200 px-4 py-3 dark:border-neutral-800">
                  <div>
