      ...
      <section className="flex flex-1 flex-col gap-8 py-10">
        <div className="space-y-3">
          ...
          {lease.status !== "draft" ? (
            <p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
              Ce bail est déjà activé. Ranti ne permet pas de le modifier librement pour protéger l'historique des loyers.
            </p>
          ) : null}
        </div>
        ...
        <form action={updateLease} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <input id="monthly_rent_amount" name="monthly_rent_amount" type="text" inputMode="numeric" required defaultValue={String(lease.monthly_rent_amount)} disabled={lease.status !== "draft"} className={inputClass} />
          </div>
          <div className="space-y-2">
            <label htmlFor="due_day" className={labelClass}>Jour d'échéance</label>
            <input id="due_day" name="due_day" type="number" min={1} max={31} required defaultValue={lease.due_day} disabled={lease.status !== "draft"} className={inputClass} />
          </div>
          <div className="space-y-2">
            <label htmlFor="start_date" className={labelClass}>Date de début</label>
            <input id="start_date" name="start_date" type="date" required defaultValue={lease.start_date} disabled={lease.status !== "draft"} className={inputClass} />
          </div>
          <div className="space-y-2">
            <label htmlFor="end_date" className={labelClass}>Date de fin <span className="text-neutral-400">(optionnel)</span></label>
            <input id="end_date" name="end_date" type="date" defaultValue={lease.end_date ?? ""} disabled={lease.status !== "draft"} className={inputClass} />
          </div>
          <div className="space-y-2">
            <label htmlFor="notes" className={labelClass}>Note <span className="text-neutral-400">(optionnel)</span></label>
            <textarea id="notes" name="notes" rows={3} defaultValue={lease.notes ?? ""} disabled={lease.status !== "draft"} className={inputClass} />
          </div>
          <SubmitButton disabled={lease.status !== "draft"} className="w-full rounded-xl bg-neutral-950 px-5 py-3 text-sm font-medium text-white transition hover:bg-neutral-800 disabled:opacity-60 dark:bg-neutral-50 dark:text-neutral-950 dark:hover:bg-neutral-200">Enregistrer</SubmitButton>
        </form>
      </section>
    </main>
  )
