    ...
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col px-6 py-8">
      <header className="flex items-center justify-between gap-4 border-b border-neutral-200 pb-5 dark:border-neutral-800">
        <div>
          ...
          <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">Vos lieux</p>
        </div>
      ...
      <section className="flex flex-1 flex-col gap-8 py-12">
        ...
        {properties.length === 0 ? (
          <div className="rounded-3xl border border-neutral-200 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-950">
            <h2 className="text-xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">Aucun lieu pour le moment</h2>
            ...
            <Link href="/properties/new" className="mt-5 inline-flex rounded-xl bg-neutral-950 px-5 py-3 text-sm font-medium text-white transition hover:bg-neutral-800 dark:bg-neutral-50 dark:text-neutral-950 dark:hover:bg-neutral-200">
          ...
          <div className="space-y-4">
            {properties.map((property) => (
              <Link key={property.id} href={`/properties/${property.id}`} className="block rounded-3xl border border-neutral-200 bg-white p-6 transition hover:border-neutral-950 dark:border-neutral-800 dark:bg-neutral-950 dark:hover:border-neutral-50">
                <h2 className="text-xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">{property.name}</h2>
                <p className="mt-2 text-base leading-7 text-neutral-600 dark:text-neutral-300">
                  {formatLocation(property.city, property.address)}
                </p>
                {property.notes ? <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-400">{property.notes}</p> : null}
                <span className="mt-5 inline-flex text-sm font-medium text-neutral-950 underline-offset-4 dark:text-neutral-50">Voir le détail</span>
              </Link>
            ))}

            <Link href="/properties/new" className="inline-flex rounded-xl border border-neutral-300 px-5 py-3 text-sm font-medium text-neutral-800 transition hover:border-neutral-950 dark:border-neutral-700 dark:text-neutral-100 dark:hover:border-neutral-50">
