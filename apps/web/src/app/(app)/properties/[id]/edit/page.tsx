    ...
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col px-6 py-8">
      <header className="flex items-center justify-between gap-4 border-b border-neutral-200 pb-5 dark:border-neutral-800">
        <div>
          ...
          <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">Modifier le lieu</p>
        </div>
        <Link href={`/properties/${property.id}`} className="text-sm font-medium text-neutral-600 underline-offset-4 hover:underline dark:text-neutral-300">
          Retour
        </Link>
      </header>
      ...
      <section className="flex flex-1 flex-col gap-8 py-10">
        <div className="space-y-3">
          <h1 className="text-3xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">Corriger ce lieu</h1>
        ...
        <form action={updateProperty} className="space-y-5">
          ...
          <div className="space-y-2">
            <label htmlFor="city" className={labelClass}>Ville <span className="text-neutral-400">(optionnel)</span></label>
            <input id="city" name="city" type="text" defaultValue={property.city ?? ""} className={inputClass} />
          </div>
          <div className="space-y-2">
            <label htmlFor="address" className={labelClass}>Adresse ou repère <span className="text-neutral-400">(optionnel)</span></label>
