    ...
    <main className="mx-auto flex min-h-screen w-full max-w-2xl flex-col px-6 py-8">
      <header className="flex items-center justify-between gap-4 border-b border-neutral-200 pb-5 dark:border-neutral-800">
        ...
        <div className="flex items-center gap-3">
          <a href={`/receipts/${receipt.id}/pdf`} className="rounded-xl bg-neutral-950 px-4 py-2 text-sm font-medium text-white transition hover:bg-neutral-800 dark:bg-neutral-50 dark:text-neutral-950 dark:hover:bg-neutral-200">Télécharger le PDF</a>
          <Link href="/receipts" className="text-sm font-medium text-neutral-600 underline-offset-4 hover:underline dark:text-neutral-300">Tous les documents</Link>
        </div>
      </header>
        ...
        <article className="rounded-3xl border border-neutral-200 bg-white p-7 dark:border-neutral-800 dark:bg-neutral-950">
          <div className="flex items-start justify-between gap-4 border-b border-neutral-200 pb-5 dark:border-neutral-800">
            ...
            <div className="text-right">
              ...
              {receipt.status === "cancelled" ? <span className="mt-1 inline-flex rounded-lg border border-red-300 px-2 py-0.5 text-xs font-medium text-red-700 dark:border-red-800 dark:text-red-200">{statusLabels[receipt.status]}</span> : null}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 border-b border-neutral-200 py-5 dark:border-neutral-800">
            <div>
              ...
              <p className="mt-1.5 text-sm font-medium text-neutral-950 dark:text-neutral-50">{landlord.first_name} {landlord.last_name}</p>
              <p className="text-sm text-neutral-600 dark:text-neutral-300">Propriétaire</p>
              {landlord.phone ? <p className="text-sm text-neutral-600 dark:text-neutral-300">{landlord.phone}</p> : null}
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.08em] text-neutral-400">À</p>
              <p className="mt-1.5 text-sm font-medium text-neutral-950 dark:text-neutral-50">{snap.tenant ? `${snap.tenant.first_name} ${snap.tenant.last_name}` : "Locataire"}</p>
              <p className="text-sm text-neutral-600 dark:text-neutral-300">Locataire</p>
              {snap.unit ? <p className="text-sm text-neutral-600 dark:text-neutral-300">{snap.unit.name}</p> : null}
            </div>
          </div>

          {snap.allocations && snap.allocations.length > 0 ? (
            <div className="border-b border-neutral-200 py-5 dark:border-neutral-800">
              <div className="flex justify-between text-xs text-neutral-400"><span>Période réglée</span><span>Montant</span></div>
              {snap.allocations.map((a, i) => (
                <div key={i} className="flex justify-between gap-4 py-1.5 text-sm">
                  ...
                  <span className="text-neutral-950 dark:text-neutral-50">{formatFcfa(a.amount_allocated)}</span>
                </div>
              ))}
            </div>
          ) : null}
          ...
          <div className="flex items-center justify-between gap-4 border-b border-neutral-200 py-5 dark:border-neutral-800">
            <div>
              ...
              {snap.reception ? <p className="text-sm text-neutral-600 dark:text-neutral-300">{methodLabels[snap.reception.payment_method] ?? snap.reception.payment_method} · reçu le {formatDate(snap.reception.received_at)}</p> : null}
            </div>
          ...
          <div className="flex items-end justify-between gap-4 pt-2">
            <div className="flex items-center gap-3">
              <span className="flex h-16 w-16 items-center justify-center rounded-lg border border-neutral-300 text-xs text-neutral-400 dark:border-neutral-700">QR</span>
              <span className="max-w-[140px] text-xs text-neutral-400">Vérifier l&apos;authenticité en ligne (bientôt)</span>
            </div>
            <div className="text-center"><div className="w-40 border-t border-neutral-300 pt-1.5 text-xs text-neutral-400 dark:border-neutral-700">Signature du propriétaire</div></div>
          </div>
        </article>

        {receipt.status === "cancelled" ? <p className="text-sm text-neutral-500 dark:text-neutral-400">Motif d&apos;annulation : {receipt.cancellation_reason ?? "Motif non renseigné avant correction."}</p> : null}
