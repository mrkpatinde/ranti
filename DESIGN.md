...
| 2026-07-22 | Minimalisme façon Moneco (dashboard, réglages, nav mobile) | Référence CEO (captures Moneco), « niveau de minimalisme voulu ». Identité intacte : Fraunces, olive, crème, tokens et `direction-artistique.html` inchangés ; on emprunte l'espace et la hiérarchie, pas les couleurs/polices. Dashboard : chiffre héro « Reste à encaisser » (Σ dû certain) + toggle œil, trio de boutons ronds d'action (Encaisser/Relancer/Quittances), pastilles rondes d'icône sur les listes, un seul guide onboarding à la fois. Les boutons ronds sont une **exception cadrée** à l'interdit « pas de bulles / pills réservées aux CTA », réservée à ce trio primaire. Nav mobile : barre d'onglets fixe en bas (remplace le hamburger), sidebar desktop inchangée. Non-custodial (ADR-019/024) respecté : aucun bouton ne déplace de fonds (Encaisser = écriture au grand livre). Puce « Gagne 5$ » écartée (pas de parrainage réel, honnêteté DESIGN.md) |
