...
## Color
> Palette de référence = **direction-artistique.html** (décision CEO 2026-07-17 : on garde les couleurs de la DA). L'app utilise des **tokens sémantiques HSL** (`globals.css` `@theme`, style shadcn) qui portent cette palette. Mapping : `--background` = papier crème **#f7f7f2**, `--foreground`/`--primary` = encre #292929, `--accent` = olive #5b6f00 (CTA), `--secondary` = teinte verte #f2f6e1, `--destructive` = #e95d3d, `--warning` = #bd4a30 (retard/attention). Dark mode câblé (accent olive éclairci **#aebd4a**, conforme à la section Dark mode ci-dessous). **Utiliser les classes tokens** (`bg-accent`, `text-foreground`, `bg-warning/10`…) dans l'app, PAS des hex en dur ni la palette Tailwind générique (`red-*`, `amber-*` interdites — elles cassent le dark mode). La landing, elle, hardcode encore les hex (dette : à migrer vers les tokens un jour).
> ⚠ Écart code connu : `globals.css` sert actuellement `#f9f8f6` en fond (patch terrain 2026-07-16, dalles Android verdâtres). La décision 2026-07-17 le supersède — réaligner le code sur `#f7f7f2` à la prochaine passe CSS (ne pas propager `#f9f8f6` dans de nouvelles surfaces).
...
| 2026-07-17 | direction-artistique.html publiée, couleurs DA verrouillées | Style-guide rendu depuis DESIGN.md + tokens prod (v0.3.26.0). Décision CEO : on garde les couleurs de la DA — papier `#f7f7f2` redevient la référence, supersède le patch terrain `#f9f8f6` du 2026-07-16 (réalignement CSS à planifier, docs adaptées en attendant) |
