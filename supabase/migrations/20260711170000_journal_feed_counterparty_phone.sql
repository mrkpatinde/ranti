-- ADR-014 — journal_feed : téléphone de la contrepartie pour la notification
-- WhatsApp sortante (étape 6 du flux collage SMS).
--
-- La timeline génère un lien profond wa.me vers le locataire depuis la ligne
-- d'encaissement. Le lien a besoin du numéro du locataire ; on l'expose ici pour
-- que la vue reste la seule source et la page un simple rendu. security_invoker
-- inchangé : la RLS du propriétaire s'applique via les jointures tenants.
--
-- wa.me n'est qu'un lien profond ouvert dans WhatsApp natif : zéro API de
-- diffusion, donc aucun coût ni risque de bannissement (principe ADR-014).
--
-- Note : counterparty_phone est ajouté EN DERNIÈRE colonne. « create or replace
-- view » interdit d'insérer une colonne au milieu (renommerait les suivantes) ;
-- l'ajout en fin de liste est le seul changement accepté sans DROP.
